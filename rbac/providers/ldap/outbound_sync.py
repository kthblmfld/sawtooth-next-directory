...
def process_entry(outbound_entry, rethink_db):
    """
    ...
    if connected_to_ldap:
        ...
        try:
            if outbound_entry["data_type"] == "user":
                ...
                remove_db_entry(outbound_entry, rethink_db)
...
def remove_db_entry(entry, rethink_db):
    """Remove entry from queue outbound table in RethinkDB."""
    entry_id = entry["id"]
    rethink_db.table(OUTBOUND_DB_TABLE).get(entry_id).delete()
